...
class log_reg_model(object):
	def __init__(self, cell_type, raw_features, raw_labels, config, log_path=None, root_dir=None, hist_marks=False, timeit=False):
		if timeit:
		...
		self.config = read_config('logreg_config.txt', self.config)
		...
		if root_dir == None:
			self.root_folder = os.path.join('logregmodels', self.script_start_time_id)
		else:
		...
		if log_path == None:
			self.script_log_path = os.path.join(self.root_folder, 'logreg_{}_LOG.csv'.format(self.script_start_time_id))
		else:
		...
		self.script_parms = ['cell', 'chrom', 'ep_state', 'acc', 'auprc', 'auroc', 'adj_rand_idx', 'f1_score', 'true_in_train', 'tot_train_samples', 'elapsed_time']
	...
	def build_model(self, timeit=False):
		...
		self.clf = LogisticRegression(random_state=16, solver=self.config['solver'], max_iter=5000, tol=1e-2, C=0.5)
	...
	def train(self):
		...
		pred_probs = self.clf.predict_proba(self.test_x) #, self.test_y)
		pred_classes = np.argmax(pred_probs, axis=1).reshape(-1, 1)
		acc_test = np.mean(self.test_y == pred_classes)

		auprc = self.calc_auprc(self.test_y_state[:,1], pred_probs[:,1])
		auroc = self.calc_auroc(self.test_y_state[:,1], pred_probs[:,1])
		...
		#self.script_parms = ['cell', 'chrom', 'ep_state', 'acc', 'auprc', 'auroc', 'adj_rand_idx', 'f1_score', 'true_in_train', 'tot_train_samples', 'elapsed_time']
		these_parms = [self.cell_type, self.config['chrom_id'], self.state, acc_test, auprc, auroc, ari, f1, num_true_in_train, tot_train_samples, model_elapsed_time]

		print('Cell: {0} State: {1} Accuracy: {2:3.2f} AUROC: {3:.4f} AUPRC: {4:.4f} Elapsed Time: {5:6.2f}s'.format(self.cell_type, str(self.state), acc_test, auroc, auprc, time.time() - epoch_start_time))
		...
		write_data = np.concatenate([self.test_y_state[:,1].reshape(-1, 1), binary_preds, pred_probs], axis=1)
		...
		print('Saving model...')
		with open(self.config['model_save_path_tmplt'].format(self.cell_type, self.state, self.config['hist_marks']), 'wb') as f:
